...
if ( ! empty( $_POST['action'] ) && ! empty( $_POST['wow_support_field'] ) ) {
	if ( wp_verify_nonce( $_POST['wow_support_field'], 'wow_support_action' ) && current_user_can( 'manage_options' ) ) {
		...
		if ( count( $error ) == 0 ) {
			...
			$message = isset($_POST['wow-message']) ? sanitize_textarea_field($_POST['wow-message']) : '';
			$type    = isset($_POST['wow-message-type']) ? sanitize_text_field($_POST['wow-message-type']) : '';
...
<?php if ( count( $error ) > 0 ) {
	echo '<div class="wow-alert wow-alert-error "><p class="wow_error">' . implode( "<br />", $error ) . '</p></div>';
} ?>
<div class="wowbox" style="width:80%;">
    <form method="post" action="">
        ...
        <div class="wow-admin-col">
            <div class="wow-admin-col-6">
                WebSite:<br/>
                <input type="text" disabled name="wow-website" value="<?php echo esc_url( get_option( 'home' ) ); ?>">
            </div>
            <div class="wow-admin-col-6">
                Contact email:<br/>
        ...
        <div class="wow-admin-col">
            <div class="wow-admin-col-6">
                ...
                <input type="text" disabled name="wow-plugin" value="<?php if ( ! empty( $name ) ) {
					echo esc_attr( $name ) . ' v.' . esc_attr( $version );
				}; ?>">
            </div>
            <div class="wow-admin-col-6">
                Message type:<br/>
